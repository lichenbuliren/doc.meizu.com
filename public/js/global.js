...
(function($, Util) {
    var GLOBAL = {
        version: '1.0.0',
        loginUrl: 'https://member.meizu.com/login/login.html?appuri=http://store-server.meizu.com/member/login&apptype=store',
        // 判断是否登录
        checkLogin: function() {
            // TODO 具体判断sessionid
            var user = Util.Cookie.get('MZ_SERVER_SESSION_VAL'),
                isLogin = Util.Cookie.get('_islogin');
            if (!isLogin || !user) {
                return false;
            }
            var user_arr = $.parseJSON(decodeURIComponent(user));
